...
`call()` 方法使用一个**指定的 `this` 值**和单独给出的一个或多个**参数**来调用一个函数。
...
![image-20220129193930143](https://gitee.com/su-fangzhou/blog-image/raw/master/202201291939287.png)
...
之前还有方法用来判断arr是否为数组,那步没啥意义 它抛出错误会显示为—— `arr._reduce is not a function`——毕竟是写在数组原型上的方法 不用数组调肯定是不行的 这个不用额外判断了~
